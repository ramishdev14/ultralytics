...
def build_environment_record(
    ...
    return {
        ...
        "model_source_file": (
            get_optional_file_record(Path(model_source))
            if Path(model_source).exists()
            else {
                "available": False,
                "path": model_source,
                "sha256": None,
                "reason": "Ultralytics model identifier or unavailable local file.",
            }
        ),
        "model_architecture_file": (
            get_optional_file_record(Path(model_architecture))
            if model_architecture is not None
            else {
                "available": False,
                "path": None,
                "sha256": None,
                "reason": "No separate architecture YAML was configured.",
            }
        ),
        "training_arguments": (
            training_arguments
...
def print_experiment_summary(
    ...
    print(
        "Model architecture: "
        f"{model_architecture if model_architecture is not None else 'checkpoint architecture'}"
    )
    print(
        f"Pretrained weights: {model_source}"
    )
    print(
...
def main() -> None:
    ...
    architecture_value = configuration["model"].get(
        "architecture"
    )
    ...
    if model_architecture is not None:
        print(
            "Creating model from custom architecture YAML: "
        ...
        print(
            "Creating model directly from checkpoint/identifier: "
            f"{model_source}"
        )
    ...
    if (
        ...
        if not best_checkpoint_path.exists():
            raise FileNotFoundError(
                ...
                "custom-architecture verification: "
